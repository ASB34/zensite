...
export default function TinyLiving() {
  ...
  const dynamicContent = useDynamicPage('tinyLiving');

  const specList = dynamicContent?.specs || t('tinyLiving.specs', { returnObjects: true }) as Array<{ label: string, value: string }>;
  ...
  return (
    <div className="flex-1 flex flex-col py-12 px-6 md:px-12 relative">
      ...
      <div className="max-w-7xl mx-auto w-full z-10">
        ...
        >
          <span className="text-cyan-400 text-[10px] uppercase font-bold tracking-widest px-3 py-1 bg-cyan-400/10 border border-cyan-400/20 rounded-full">{dynamicContent?.tag || t('tinyLiving.tag')}</span>
        </motion.div>
        ...
        >
          {dynamicContent?.title1 || t('tinyLiving.title1')}<span className="font-medium">{dynamicContent?.title2 || t('tinyLiving.title2')}</span> {dynamicContent?.title3 || t('tinyLiving.title3')}
        </motion.h1>
          ...
          <div className="relative h-[400px] md:h-[500px] rounded-2xl bg-gradient-to-br from-[#1A1A1A] to-[#0A0A0A] border border-white/10 overflow-hidden shadow-2xl flex items-center justify-center group">
             ...
             <div className="relative w-3/4 h-1/3 bg-white/5 border border-white/20 rounded-xl flex preserve-3d shadow-[0_20px_50px_rgba(0,0,0,0.5)] transform -rotate-12 hover:rotate-0 transition-transform duration-700 ease-out">
                ...
                <div className="w-1/2 h-full border-r border-white/10 relative flex items-center justify-center overflow-hidden">
                   <div className="absolute top-4 left-4 text-[8px] text-cyan-400 uppercase tracking-widest opacity-60">{dynamicContent?.diagramTag1 || t('tinyLiving.diagramTag1')}</div>
                ...
                <div className="w-1/2 h-full relative p-4 flex items-center justify-center bg-white/5">
                   <div className="absolute top-4 left-4 text-[8px] text-gray-400 uppercase tracking-widest opacity-60">{dynamicContent?.diagramTag2 || t('tinyLiving.diagramTag2')}</div>
             ...
             <div className="absolute bottom-6 left-6 right-6 flex justify-between text-[10px] uppercase tracking-widest text-white/40">
               <span>{dynamicContent?.feature1 || t('tinyLiving.feature1')}</span>
               <span>{dynamicContent?.feature2 || t('tinyLiving.feature2')}</span>
             </div>
          </div>

          {/* Copy and Specs */}
          <div className="flex flex-col">
            <h3 className="text-2xl font-light mb-4">{dynamicContent?.heading || t('tinyLiving.heading')}</h3>
            <p className="text-sm opacity-60 font-light leading-relaxed mb-8">
              {dynamicContent?.desc || t('tinyLiving.desc')}
            </p>
            ...
            <button className="self-start px-8 py-4 bg-cyan-400 hover:bg-cyan-300 text-black rounded-lg text-xs uppercase font-bold tracking-[0.2em] transition-all shadow-[0_0_20px_rgba(34,211,238,0.3)] hover:shadow-[0_0_30px_rgba(34,211,238,0.5)]">
              {dynamicContent?.btn || t('tinyLiving.btn')}
            </button>
          </div>
        </motion.div>
      </div>
